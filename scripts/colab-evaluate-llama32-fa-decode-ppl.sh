...
# Score one fixed held-out target window with built-in and custom Q=1 decode paths.
...
python3 - "$O/builtin.txt" "$O/custom.txt" "$O/summary.txt" "$ROUTE_COUNT" "$PREFILL" "$SCORE" "$START" <<'PY'
import sys
