...
# splitk route tracing. A separate untimed traced splitk probe proves dispatch.
[[ $# -ge 6 ]] || {
...
mkdir -p "$O"
{
    echo "Phase 6 Llama 3.2 Split-K FD benchmark"
    echo "UTC: $(date -u --iso-8601=seconds)"
    nvidia-smi --query-gpu=name,driver_version,memory.total --format=csv,noheader || true
} > "$O/environment.txt"
echo "context,prompt_tokens,timed_decode_steps,timed_runs,baseline_mean_ms,baseline_std_ms,baseline_tok_s,splitk_mean_ms,splitk_std_ms,splitk_tok_s,speedup,splitk_probe_routes" > "$O/summary.csv"
...
for C; do
    ...
    env GGML_CUDA_LLAMA32_FA_DECODE_ENABLED=1 GGML_CUDA_LLAMA32_FD_SPLITK_ENABLED=1 GGML_CUDA_LLAMA32_FA_DECODE_TRACE=1 \
        ...
        --flash-attn on > "$D/splitk-route-probe.txt" 2> "$D/splitk-route-probe.stderr"
    ROUTES="$(grep -c '^llama32-fd-splitk route: selected$' "$D/splitk-route-probe.stderr" || true)"
    [[ "$ROUTES" -gt 0 ]] || { echo "FAIL: no splitk route at context $C" >&2; exit 1; }
    ...
    env GGML_CUDA_LLAMA32_FA_DECODE_ENABLED=1 GGML_CUDA_LLAMA32_FD_SPLITK_ENABLED=1 \
        "$H" --model "$M" --prompt-file "$P" --context-tokens "$C" \
        --ctx-size "$CTX" --predict "$N" --gpu-layers "$G" \
        --batch-size "$BATCH" --ubatch-size 512 --warmup 2 --runs 5 \
        --flash-attn on > "$D/splitk.txt" 2> "$D/splitk.stderr"
    ...
    CM="$(sed -n 's/^DECODE_MEAN_MS=//p' "$D/splitk.txt")"
    CS="$(sed -n 's/^DECODE_STD_MS=//p' "$D/splitk.txt")"
    CT="$(sed -n 's/^DECODE_TOKENS_PER_SECOND=//p' "$D/splitk.txt")"
    SPEEDUP="$(awk -v splitk="$CT" -v baseline="$BT" 'BEGIN { if (baseline > 0) printf "%.6f", splitk / baseline; else print "nan" }')"

    echo "$C,$BP,$STEPS,$RUNS,$BM,$BS,$BT,$CM,$CS,$CT,$SPEEDUP,$ROUTES" >> "$O/summary.csv"
    echo "context=$C baseline_tok_s=$BT splitk_tok_s=$CT speedup="$SPEEDUP"x probe_routes=$ROUTES"
done

echo "PASS: Phase 6 decode benchmark completed after separate splitk-route probes."
echo "Results: $O"
